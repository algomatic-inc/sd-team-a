...
export const getNominatim = async (query: string) => {
...
export const getNominatimResponseJson = async (query: string) => {
  const res = await getNominatim(query);
...
export const getNominatimResponseJsonWithCache = async (
  ...
  md5.appendStr(query);
  const hash = md5.end();
  const key = `nobushi-nominatim-cache-${hash}`;
